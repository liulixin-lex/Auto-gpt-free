...
def _create_cloud_mail(extra: dict, proxy: str | None) -> BaseMailbox:
    ...
    return CloudMailMailboxPool(
        api_base=extra.get("cloud_mail_api_base", ""),
        api_key=extra.get("cloud_mail_api_key", ""),
        admin_email=extra.get("cloud_mail_admin_email", ""),
        admin_password=extra.get("cloud_mail_admin_password", ""),
...
def _create_domain_imap(extra: dict, proxy: str | None) -> BaseMailbox:
    from core.domain_imap_mailbox import DomainImapMailboxPool

    return DomainImapMailboxPool(
        domain=extra.get("domain_imap_domain", ""),
        imap_host=extra.get("domain_imap_host", ""),
        imap_user=extra.get("domain_imap_user", ""),
        imap_password=extra.get("domain_imap_password", ""),
        imap_port=extra.get("domain_imap_port", 993),
        use_ssl=extra.get("domain_imap_ssl", True),
        local_prefix=extra.get("domain_imap_prefix", "fai"),
        poll_interval=extra.get("domain_imap_poll_interval", 3),
        proxy=proxy,
    )
...
MAILBOX_FACTORY_REGISTRY = {
    ...
    "domain_imap": _create_domain_imap,
}

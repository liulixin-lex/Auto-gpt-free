...
def _cleanup_non_real_providers():
    """Remove unsupported mailbox providers (public free temp-mail + generic_http)."""
    # Always drop these — free public temp-mail is unreliable for ChatGPT OTP/survival.
    remove_keys = [
        ("mailbox", "generic_http"),
        ("mailbox", "mail_tm"),
        ("mailbox", "tempmail_lol"),
        ("mailbox", "guerrilla_mail"),
    ]
    with Session(engine) as session:

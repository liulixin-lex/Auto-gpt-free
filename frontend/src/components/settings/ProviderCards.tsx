...
function Toggle({ checked, onChange, disabled }: { checked: boolean; onChange: (v: boolean) => void; disabled?: boolean }) {
  return (
    <button
      ...
      } ${checked ? 'bg-[var(--accent)]' : 'bg-[var(--chip-bg)] border-2 border-[var(--border)]'}`}
    >
      <span className={`inline-block h-3.5 w-3.5  bg-white shadow transition-transform ${checked ? 'translate-x-[18px]' : 'translate-x-[3px]'}`} />
    </button>
  )
}
...
function SearchableSelect({ value, options, placeholder, onChange }: {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full  border-2 border-[var(--border)] bg-[var(--bg-pane)] shadow-lg">
          <div className="p-2 border-b border-[var(--border)]">
            <div className="relative">
              ...
              <input
                ...
                className="w-full  border-2 border-[var(--border)] bg-[var(--bg-base)] pl-8 pr-3 py-1.5 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-1 focus:ring-[var(--accent)]"
              />
            </div>
          </div>
  ...
  return (
    <div className="dialog-backdrop" onClick={onClose}>
      <div className="dialog-panel dialog-panel-sm flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        {/* Test result */}
        {testResult && (
          <div className={`mx-5  px-3 py-2 text-xs ${
            testResult.ok
...
export default function ProviderCards({ providerType, catalog, settings, onReload, onCreateCustom }: Props) {
  ...
  const renderCard = (provider: ProviderOption, allowDelete = false) => {
    ...
    return (
      <div key={key}>
        <div className="flex items-center gap-3  border-2 border-[var(--border)] bg-[var(--bg-card)] px-4 py-3">
        ...
        {/* Inline test result */}
        {testResults[key] && (
          <div className={`mt-1  px-3 py-2 text-xs ${
            testResults[key].ok
            ...
            <div key={cat}>
              ...
              <div className="space-y-1.5">
                ...
                {cat === 'custom' && onCreateCustom && (
                  <button
                    className="flex w-full items-center justify-center gap-2  border border-dashed border-[var(--border)] px-4 py-3 text-sm text-[var(--text-muted)] transition-colors hover:border-[var(--accent)] hover:text-[var(--accent)]"

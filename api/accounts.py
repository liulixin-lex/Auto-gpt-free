...
def export_accounts_sub2api_agent_identity(body: BatchExportRequest):
    """Prefer Agent Identity; free/disabled registry → OAuth (upstream freeAgentIdentity)."""
    try:
...
def export_accounts_sub2api(body: BatchExportRequest):
    """Upstream free-account path: Sub2API OAuth token JSON only (no agent/register)."""
    try:

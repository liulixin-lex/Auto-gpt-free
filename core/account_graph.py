...
def _stats_bucket(display_status: Any) -> str:
    """Map display_status into one exclusive pool bucket."""
    raw = _text(display_status).lower() or "registered"
    if raw in {"invalid"}:
        return "invalid"
    if raw in {"expired"}:
        return "expired"
    if raw in {"trial", "trialing"}:
        return "trial"
    if raw in {"subscribed", "pro", "plus", "paid"}:
        return "subscribed"
    # free / eligible / registered / unknown → active "registered" stock
    return "registered"


def compute_account_stats(graphs: list[dict[str, Any]], platforms: list[str]) -> dict[str, Any]:
    ...
    for graph in graphs:
        by_bucket[_stats_bucket(graph.get("display_status"))] += 1

    total = len(graphs)
    fault = int(by_bucket.get("invalid", 0)) + int(by_bucket.get("expired", 0))
    active = max(0, total - fault)

    # stable key order for clients
    ordered_buckets = {
        key: int(by_bucket.get(key, 0))
        for key in ("registered", "trial", "subscribed", "expired", "invalid")
    }
